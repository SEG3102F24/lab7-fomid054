...
export class AdminComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.booksService.addBook(book).subscribe({
      next: (response) => {
        const authors = <any[]>this.bookForm.value.authors;
        authors.forEach(
          (author: any) => {
            if (author.authorId) {
              // Use existing author
              console.log(`Using existing author ID: ${author.authorId}`);
              this.booksService.updateBookAuthors(response.id, author.authorId).subscribe();
            } else {
              // Create a new author
  ...
  addAuthor(): void {
    this.authors.push(
      this.builder.group({
        authorId: [''],   // Optional author ID for existing author
        firstName: [''],  // New author first name
        lastName: ['']    // New author last name
      })
    );
  }
