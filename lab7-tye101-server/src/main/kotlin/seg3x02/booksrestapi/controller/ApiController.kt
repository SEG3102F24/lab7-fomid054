...
class ApiController(val authorRepository: AuthorRepository,
    ...
    fun getAuthorBioById(@PathVariable("id") id: Long): ResponseEntity<BioRepresentation> {
        return authorRepository.findById(id)
            .map { author: Author ->
                author.bio?.let { bioAssembler.toModel(it) } ?: throw NoSuchElementException("Bio not found")
            }
    ...
    fun getBookById(@PathVariable("id") id: Long): ResponseEntity<BookRepresentation> {
        return bookRepository.findById(id)
            .map { entity: Book ->
                println("Fetched book with title: ${entity.title} and authors: ${entity.authors.map { it.firstName + " " + it.lastName }}")
                bookAssembler.toModel(entity)
            }
    ...
    @Operation(summary = "Add a new book with authors")
    @PostMapping("/books")
    fun addBook(@RequestBody book: Book): ResponseEntity<Any> {
        return try {
            ...
            // Save the book, which also cascades to the authors
    ...
    fun addAuthorToBook(@PathVariable("id") id: Long, @RequestBody author: Author): ResponseEntity<Any> {
        return try {
            ...
            // Fetch author from the repository to avoid detached entity issues
            val existingAuthor = authorRepository.findAuthorsByName(author.firstName, author.lastName)
                .firstOrNull() ?: authorRepository.save(author)

            if (!book.authors.contains(existingAuthor)) {
            ...
            val location: URI = ServletUriComponentsBuilder
                ...
                .path("/authors")
                .path("/{id}")
                .buildAndExpand(existingAuthor.id)
                .toUri()

            ResponseEntity.created(location).body(authorAssembler.toModel(existingAuthor))
    ...
    fun updateBookAuthors(
        @PathVariable("bid") bid: Long,
        @PathVariable("aid") aid: Long
    ): ResponseEntity<Any> {
        return try {
            val book = bookRepository.findById(bid)
                .orElseThrow { NoSuchElementException("Book with id $bid not found") }
            val author = authorRepository.findById(aid)
                .orElseThrow { NoSuchElementException("Author with id $aid not found") }
            ...
            ResponseEntity.badRequest().body("Invalid data provided")
        }
    }
